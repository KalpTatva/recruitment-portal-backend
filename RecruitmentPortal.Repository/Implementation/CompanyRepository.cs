...
public class CompanyRepository : GenericRepository<Company>, ICompanyRepository
{
    ...
    public async Task<CompanyDetailsForProfileViewModel> GetCompanyDetailsByEmailForProfile(string email)
    {
        try
        {
            CompanyDetailsForProfileViewModel company = new();
            User? user = await _context.Users
                .Where(u => u.Email == email.Trim().ToLower())
                .FirstOrDefaultAsync();
            if (user == null)
            ...
            company.Phone = companyDetails?.Phone ?? 99999999999;
            company.countryCode = companyDetails?.CountryCode ?? "";
            company.ImageUrl = companyDetails?.ImageUrl ?? "";
            ...
            List<ViewModels.CompanyLocationWithNameForProfileViewModel> companyLocations = await _context.CompanyLocations
                .Where(cl => cl.CompanyId == company.CompanyId && cl.IsDeleted == false)
                .Select(cl => new ViewModels.CompanyLocationWithNameForProfileViewModel
                {
                    CompanyLocationId = cl.CompanyLocationId,
                    CountryId = cl.CountryId,
            ...
            if (company.CompanyLocations == null)
            {
                company.CompanyLocations = new List<ViewModels.CompanyLocationWithNameForProfileViewModel>();
            }
